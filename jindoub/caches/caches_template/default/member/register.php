...
$(function(){
 ...
 //                return false;
	...
	// 	onwait : "<?php echo L('connecting_please_wait');?>"
	// });
	$("#tel").formValidator({onshow:"请输入手机号码",onfocus:"请输入手机号码"}).inputValidator({min:11,max:11,onerror:"请输入正确的手机号码"}).ajaxValidator({
	    type : "get",
		url : "",
		data :"m=member&c=index&a=public_checktel_ajax",
		datatype : "html",
		async:'false',
		success : function(data){	
            if( data == "1" ) {
...
<div id="header">
	<div class="content">
	<div class="logo"><a href="<?php echo siteurl($siteid);?>"><img src="<?php echo IMG_PATH;?>logo/logo2.png" /></a></div>
    <span class="log"></span>
    </div>
</div>
...
<form method="post" action="" id="myform">
	...
	<div class="col-left form-login form-reg">
    	...
    	<div class="input"><label><?php echo L('username');?>：</label><input type="text" id="username" name="username" size="36" class="input-text"><span style="color: red;">*</span></div>
        <div class="input"><label><?php echo L('password');?>：</label><input type="password" id="password" name="password" size="36" class="input-text"><span style="color: red;">*</span></div>
        <div class="input"><label><?php echo L('cofirmpwd');?>：</label><input type="password" name="pwdconfirm" id="pwdconfirm" size="36" class="input-text"><span style="color: red;">*</span></div>
        <div class="input"><label>电话号码：</label><input type="text" id="tel" name="tel" size="36" class="input-text"><span style="color: red;">*</span></div>
		...
		<?php if($member_setting['choosemodel']) { ?>
			...
			<!-- <?php $n=1; if(is_array($forminfos)) foreach($forminfos AS $k => $v) { ?>
				<div class="input">
				<label><?php if($v['isbase']) { ?><font color=red>*</font><?php } ?> <?php echo $v['name'];?>：<?php if($v['tips']) { ?><br />(<?php echo $v['tips'];?>)<?php } ?></label>
